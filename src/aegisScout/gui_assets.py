...
<head>
  ...
  <style>
    /* 🎨 Legacy Theme Compatibility Layer (--bg-primary / --text-primary aliases) */
    /* These 4 plain selectors complement the existing 12 `body.theme-*` themes. */
    /* The :root and body.light defaults are the LIGHT theme. */
    :root, body.light {
      --bg-primary: #f8f9fa;
      --bg-secondary: #ffffff;
      --bg-sidebar: #1a1d23;
    ...
    /* Alias the new var names to the existing 12-theme system for full theme coverage. */
    body.theme-neon, body.theme-emerald, body.theme-cyberpunk, body.theme-light,
    body.theme-midnight, body.theme-amber, body.theme-amethyst, body.theme-rose,
    body.theme-dracula, body.theme-solarized, body.theme-slate, body.theme-monochrome,
    body.dark, body.neon, body.midnight, body.light {
    ...
    * {
    ...
    .btn {
      background: linear-gradient(135deg, var(--color-brand) 0%, var(--accent-hover) 100%);
      color: white;
      border: none;
      border-radius: 12px;
      cursor: pointer;
    ...
    .badge-new { background-color: rgba(59, 130, 246, 0.15); color: var(--accent); }
    .badge-researched { background-color: rgba(245, 158, 11, 0.15); color: var(--warning); }
    .badge-drafted { background-color: rgba(168, 85, 247, 0.15); color: var(--accent); }
    .badge-contacted { background-color: rgba(16, 185, 129, 0.15); color: var(--success); }
    .badge-replied { background-color: rgba(139, 92, 246, 0.15); color: var(--accent); }
    .badge-converted { background-color: rgba(16, 185, 129, 0.25); color: var(--success); }
    .badge-rejected { background-color: rgba(239, 68, 68, 0.15); color: var(--danger); }
    .badge-do_not_contact { background-color: rgba(239, 68, 68, 0.25); color: var(--danger); border: 1px solid rgba(239,68,68,0.3); }
    ...
    .notif.success .notif-title { color: var(--success); }
    .notif.error   .notif-title { color: var(--danger); }
    ...
    .notif.success .notif-progress-bar { background: linear-gradient(90deg, var(--success), var(--success)); }
    .notif.error   .notif-progress-bar { background: linear-gradient(90deg, var(--danger), var(--danger)); }
    .notif.info    .notif-progress-bar { background: linear-gradient(90deg, var(--accent), var(--accent)); }
    .notif.warning .notif-progress-bar { background: linear-gradient(90deg, var(--warning), var(--warning)); }
    ...
    .modal-card {
      background: linear-gradient(145deg, var(--bg-secondary), var(--bg-secondary));
    ...
    .sidebar.collapsed .session-indicator,
    .sidebar.collapsed .nav-item span[data-i18n] {
      opacity: 0;
      max-width: 0;
      pointer-events: none;
    }
  ...
  <script>
    // Apply saved theme on page load to prevent FOUC.
    (function() {
      try {
        var savedTheme = localStorage.getItem('aegis-theme') || 'theme-neon';
...
<body>
  ...
  <div class="sidebar" id="app-sidebar">
    <button class="sidebar-toggle-btn" id="sidebar-toggle-btn" onclick="toggleSidebar()" data-i18n-title="sidebar_toggle_title" title="Menüyü Daralt/Genişlet" aria-label="Toggle sidebar">☰</button>
    <div class="brand">
      ...
      <div>
        <div class="brand-name" data-i18n="brand_name">aegisScout</div>
        <div class="session-indicator" id="sidebar-session-name" data-i18n="session_default_indicator">● Varsayılan Oturum</div>
      </div>
    </div>
        <ul class="nav-links">
  ...
  <div class="main-content">
    ...
    <div id="panel-dashboard" class="panel active">
      <div>
        <h1 data-i18n="dashboard_h1">Dashboard</h1>
        <p class="subtitle" data-i18n="dashboard_subtitle_text">Müşteri keşif turları başlatın ve genel durum istatistiklerini takip edin.</p>
      </div>
      ...
      <div class="stats-grid">
        <div class="stat-card">
          ...
          <div class="stat-lbl" data-i18n="stat_new_discovered">Yeni Keşfedilen</div>
        </div>
        <div class="stat-card">
          <div class="stat-val" id="count-researched">0</div>
          <div class="stat-lbl" data-i18n="stat_ai_ready">AI Analizi Hazır</div>
        </div>
        <div class="stat-card">
          <div class="stat-val" id="count-contacted">0</div>
          <div class="stat-lbl" data-i18n="stat_contacted">İletişime Geçilen</div>
        </div>
        <div class="stat-card">
          <div class="stat-val" id="count-replied">0</div>
          <div class="stat-lbl" data-i18n="stat_replied">Geri Dönüş Yapan</div>
        </div>
      </div>

      <!-- 🔎 Discovery Form Card -->
      <div class="stat-card" style="gap: 20px;">
        <h2 style="font-family: var(--font-display); font-size: 1.3rem;" data-i18n="discovery_form_title">Yeni Keşif Başlat</h2>
        <div class="form-row">
          <div class="form-group">
            <label for="search-sector" data-i18n="lbl_search_sector">Sektör / Anahtar Kelimeler</label>
            <textarea id="search-sector" rows="3" data-i18n-placeholder="ph_search_sector" placeholder="kuaför&#10;mimar&#10;diş kliniği">kuaför</textarea>
          </div>
          <div class="form-group">
            <label for="search-location" data-i18n="lbl_search_location">Konum / Şehir / Bölge / Ülke</label>
            <textarea id="search-location" rows="3" data-i18n-placeholder="ph_search_location" placeholder="Kadıköy&#10;İstanbul&#10;Türkiye">Kadıköy, İstanbul</textarea>
          </div>
          <div class="form-group">
            <label for="search-radius" data-i18n="lbl_search_radius">Yarıçap (km)</label>
            <input type="number" id="search-radius" min="0" data-i18n-placeholder="ph_search_radius" placeholder="Boş = Sınırsız" value="10">
          </div>
          <div class="form-group">
        ...
        <div class="form-row">
          <div class="form-group">
            <label for="search-country" data-i18n="lbl_search_country">Ülke(ler)</label>
            <textarea id="search-country" rows="2" data-i18n-placeholder="ph_search_country" placeholder="Türkiye&#10;Almanya"></textarea>
          </div>
          <div class="form-group">
            <label for="search-city" data-i18n="lbl_search_city">Şehir(ler)</label>
            <textarea id="search-city" rows="2" data-i18n-placeholder="ph_search_city" placeholder="İstanbul&#10;Berlin"></textarea>
          </div>
          <div class="form-group">
            <label for="search-region" data-i18n="lbl_search_region">Bölge / İlçe</label>
            <textarea id="search-region" rows="2" data-i18n-placeholder="ph_search_region" placeholder="Marmara&#10;Kadıköy"></textarea>
          </div>
          <div class="form-group">
            <label for="search-notes" data-i18n="lbl_search_notes">Taslak Notu</label>
            <input type="text" id="search-notes" data-i18n-placeholder="ph_search_notes" placeholder="Örneğin: premium odak, yeni açılan işletmeler...">
          </div>
        </div>
        <div class="form-row">
          <div class="form-group">
            <label for="search-preset-name" data-i18n="lbl_search_preset_name">Taslak / Preset Adı</label>
            <input type="text" id="search-preset-name" data-i18n-placeholder="ph_search_preset_name" placeholder="Örneğin: İstanbul kuaför taraması">
          </div>
          <div class="form-group">
            <label for="search-preset-select" data-i18n="lbl_search_preset_select">Kayıtlı Taslaklar</label>
            <select id="search-preset-select">
              <option value="" data-i18n="opt_select_preset">Yüklenecek taslak seçin</option>
            </select>
          </div>
          <div class="form-group">
            <label for="search-draft-select" data-i18n="lbl_search_draft_select">Kayıtlı Arama Taslakları</label>
            <select id="search-draft-select">
              <option value="" data-i18n="opt_select_draft">Taslak seçin</option>
            </select>
          </div>
          <div class="form-group" style="justify-content: end;">
            <label>&nbsp;</label>
            <div style="display: flex; gap: 8px; flex-wrap: wrap;">
              <button class="btn btn-secondary" type="button" onclick="saveSearchPresetFromUI()" data-i18n="btn_save_preset">Preset Kaydet</button>
              <button class="btn btn-secondary" type="button" onclick="saveSearchDraftFromUI()" data-i18n="btn_save_draft">Taslak Kaydet</button>
              <button class="btn btn-secondary" type="button" onclick="loadSelectedSearchPreset()" data-i18n="btn_load_preset">Preset Yükle</button>
              <button class="btn btn-secondary" type="button" onclick="loadSelectedSearchDraft()" data-i18n="btn_load_draft">Taslak Yükle</button>
            </div>
          </div>
        </div>
        <div>
          <button class="btn" id="btn-start-discovery" onclick="triggerDiscovery()">
            <span id="discovery-btn-text" data-i18n="btn_start_discovery">Keşfi Başlat</span>
        ...
        <div id="discovery-progress-box" class="hidden" style="margin-top: 16px; padding: 16px; background-color: var(--bg-surface); border: 1px solid var(--border-subtle); border-radius: 12px;">
          <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
            <span style="font-weight: 600; font-size: 0.9rem; color: var(--color-accent);" data-i18n="lbl_discovery_progress">Keşif İlerleme Durumu</span>
  ...
  <script>
    ...
    /* ══════════════════════════════════════════════
       ...
       ══════════════════════════════════════════════ */
    // Persists the collapsed state under the key 'aegis-sidebar-collapsed'.
    // On page load, restoreSidebarState() reads this value and applies the
    // .collapsed class on the sidebar element so the user's preference survives
    // a reload or restart of the desktop app.
    ...
    function toggleSidebar() {
      ...
      try {
        safeLocalStorage.setItem(SIDEBAR_COLLAPSED_KEY, willCollapse ? '1' : '0');
      } catch (_) { /* localStorage may be disabled — non-fatal */ }
      // Repaint toggle button title attribute for accessibility in current language
      try {
        const btn = document.getElementById('sidebar-toggle-btn');
        if (btn) {
          btn.setAttribute('title', getTranslation('sidebar_toggle_title'));
          btn.setAttribute('aria-label', getTranslation('sidebar_toggle_title'));
        }
      } catch (_) {}
    }
    ...
    function restoreSidebarState() {
      try {
        const raw = safeLocalStorage.getItem(SIDEBAR_COLLAPSED_KEY);
        if (raw === '1') {
          const sidebar = document.getElementById('app-sidebar');
          if (sidebar) sidebar.classList.add('collapsed');
        }
      } catch (_) { /* ignore — default to expanded */ }
    }
    ...
    function applyLanguage(langId) {
      ...
      // Also translate elements using data-i18n-title / data-i18n-placeholder
      // for HTML attributes (title, placeholder) that don't get touched above.
      document.querySelectorAll('[data-i18n-title]').forEach(el => {
        const key = el.getAttribute('data-i18n-title');
        if (lang[key]) el.setAttribute('title', lang[key]);
      });
    ...
    function _initApp() {
      ...
      // 🧭 Restore sidebar collapsed/expanded state from previous session
      try { restoreSidebarState(); } catch (_) {}
